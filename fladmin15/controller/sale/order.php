...
class ControllerSaleOrder extends Controller {
  private $error = array();
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      //echo "test".$this->request->post['sendsms']; die;     

    /*  if($this->request->get['order_id']) //code for order status change 
        {

        $order_status_info=$this->model_sale_order->getorder_statusdetails($this->request->get['order_id']); 
        $order_status_name=$this->model_sale_order->getorder_statusname($this->request->post['order_status_id']);  

         
        
        if($this->request->post['order_status_id'] != $order_status_info[0]) 
        {
          if($this->request->post['sendsms'] == 'yes')
          { $order_status_info[1]=$this->request->post['payment_address_2'];
            
            if($order_status_name == 'Processed') //received
          {
            $dynmsg="Your Order ID: ".$this->request->get['order_id']." has been processed. A shipment notice will be sent shortly - Team FL (+91-91768-70701)"; 
          }
          else if($order_status_name == 'Shipped') //processed
          {
            $dynmsg="Your Order ID: ".$this->request->get['order_id']." is shipped. Way Bill Number is XXXX. To track your order click on https://www.bluedart.com/maintracking.html - Team FL (+91-91768-70701)"; 
            
          }
          else if($order_status_name == 'Completed')
          {
            $dynmsg="Your Order ID: ".$this->request->get['order_id']." is delivered. Leave us your valuable Review on Facebook (www.facebook.com/footlounge.online). Get a non-expiring EXTRA 10% off coupon for your next purchase. Team FL (+91-91768-70701)"; 
          }  

            $sendsms=new sendsms("http://dnd.bdsindia.net/api/v3",'sms', "A5fc55c66175beae4d7ccd6688bcb1085", "BDSGrp");
    ...
    $this->data['entry_track_order'] = $this->language->get('entry_track_order');
    $this->data['entry_way_billno'] = $this->language->get('entry_way_billno');
...
if (isset($this->error['payment_address_2'])) {
  ...
  if ((utf8_strlen($this->request->post['payment_address_2']) < 10) || (utf8_strlen($this->request->post['payment_address_2']) > 12)) {
  ...
  public function upload() {
    ...
    {   
      ...
      $order_status_info=$this->model_sale_order->getorder_statusname($this->request->post['orderstatusid']);  
      echo $order_status_info;   
    }
    ...
    public function updateorder()
    {
      ...
      echo json_encode($this->model_sale_order->updateorderinfo($orderid));  
      
      
    }
    public function getcustomerdetails()//get customer details
    {
      $orderid=$this->request->post['orderid'];
      
      $this->load->model('sale/order'); 
      echo $this->model_sale_order->getcustomerinfos($orderid);  
    ...
    if($this->request->post['order_id']) //code for order status change 
        {

        $order_status_info=$this->model_sale_order->getorder_statusdetails($this->request->post['order_id']);  
        $order_status_name=$this->model_sale_order->getorder_statusname($this->request->post['orderstatusid']);    
        
        $order_status_name=trim($order_status_name);
        
          
          if($order_status_name == 'Processed' || $order_status_name == 'Shipped' || $order_status_name == 'Completed' )
          { 

          

            $order_status_info[1]=$this->request->post['payment_address_2'];
            $order_status_info[1]=trim($order_status_info[1]);
             $sms_waybillno='';           $linkthis='';            


            if($order_status_name == 'Processed') //received
          {
            $dynmsg="Your Order ID: ".$this->request->post['order_id']." has been processed. A shipment notice will be sent shortly - Team FootLounge (+91-91768-70701)"; 
          }
          if($order_status_name == 'Shipped') //processed
          {
            ...
            if($this->request->post['waybillno']=='')
             {$waybillno='XXX';} else 
            {$waybillno=$this->request->post['waybillno'];$sms_waybillno=$this->request->post['waybillno'];}

           /* if($this->request->post['order_track']=='bluedart')
            {
              $linkthis='https://www.bluedart.com/maintracking.html';

            }*/
            if($this->request->post['order_track']=='dtdc')
            {
             $linkthis='http://www.dtdctrack.in/dtdc-tracking-track-courier-consignment-status/';

            }
            else if($this->request->post['order_track']=='fedex')
            {
              $linkthis='http://www.fedex.com/in/domestic/track/';
            }
            
            else if($this->request->post['order_track']=='trackcourier')
            {
              $linkthis='http://www.trackcourier.in/tracking-delhivery.php';
            }
            else if($this->request->post['order_track']=='aramex')
            {
