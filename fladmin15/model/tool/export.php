...
class ModelToolExport extends Model {
	...
	function uploadOptionsIndividual( &$reader, &$database ) 
	{
...
// find the default language id
	...
	if ($model=="") {
       ...
       $productId=trim($result->row['product_id']);



			//$productId1= trim($this->getCell($data,$i,$j++));
			...
			$options[$i] = array();
			$options[$i]['product_id'] = $productId;
			$options[$i]['option_value_id'] = $option_value_id;
			$options[$i]['type'] = $type;
			$options[$i]['value'] = $value;
			$options[$i]['image'] = $image;
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || ($type=='image')) {
	...
	return $this->storeOptionsIntoDatabaseIndividual( $database, $options,$errorArray );
	}

	function storeOptionsIntoDatabaseIndividual( &$database, &$options,&$errorArray ) 
	{
		// find the default language id
...
//print_r($options);die;
      ...
      if(++$i === $numItems) {
                        ...
                        $sql  = "UPDATE `".DB_PREFIX."product` SET `quantity`='".$totQuantity."' WHERE `product_id`=$pid";
	...
	$countrowrecord++;
			...
			$sql = "SELECT * FROM `".DB_PREFIX."product_option_value` WHERE `option_value_id`=$option_value_id AND `product_id`=$productId";
		...
		if($result2->num_rows > 0)
		{
			...
			$sql .= "WHERE `option_value_id`=$option_value_id AND `product_id`=$productId" ;
		...
		$result = $database->query( $sql );
				...
				$prevoption_value_id=$result->row['option_value_id'];
				...
				$sql = "SELECT * FROM `".DB_PREFIX."product_option_value` WHERE `option_value_id`=$prevoption_value_id AND `product_id`=$productId";
				
					$result1 = $database->query( $sql );
					if($result1->num_rows > 0)
					{ 
						$product_option_id=$result1->row['product_option_id'];
						$option_id=$result1->row['option_id'];
						$productid=$result1->row['product_id'];

						$sql1 = "insert into `".DB_PREFIX."product_option_value` ( `product_option_id`, `product_id`, `option_id`, `option_value_id`, `quantity`, `subtract`, `price`, `price_prefix`, `points`, `points_prefix`, `weight`, `weight_prefix`) values($product_option_id,$productid,$option_id,$option_value_id,$quantity,1,'0.0000','+',0,'+','0.00000000','+')";
...
$totQuantity=$totQuantity+ $option['quantity'];
...
array_push($returnArray,$responseArray,$errorArray);
	...
	function sizevalidateHeading( &$data, &$expected ) {
		$heading = array();
		$k = PHPExcel_Cell::columnIndexFromString( $data->getHighestColumn() );
   ...
   protected function sizeexpectedOptionsHeading() {
				return array( "model","option_value_id","option", "type", "value", "image",  "quantity");
	}
	...
	function validateOptionsIndividual( &$reader )
	{
		$sizeexpectedOptionsHeading = $this->sizeexpectedOptionsHeading();
		$data = $reader->getSheet(0);

				//print_r($data  );die;

		return $this->sizevalidateHeading( $data, $sizeexpectedOptionsHeading );
	}
	...
	function validateSizeUpload( &$reader )
	{

            


		$expectedWorksheets = array('Options');
		$sheetNames = $reader->getSheetNames();
		...
		if (!$this->validateOptionsIndividual( $reader )) {
	...
	function size_upload( $filename ) {
		...
		try {
			...
			// read the various worksheets and load them to the database
			$ok = $this->validateSizeUpload( $reader );
			...
			$ok = $this->uploadOptionsIndividual( $reader, $database );
...
protected function sizegetOptions( &$database, $languageId ) {
		...
		$query .= "LEFT JOIN `".DB_PREFIX."product_option_value` pov ON pov.product_option_id = po.product_option_id ";
		$query .= "LEFT JOIN `".DB_PREFIX."product` p ON p.product_id=pov.product_id ";
	...
	protected function sizepopulateOptionsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, $textFormat )
	{
		// Set the column widths
		...
		$options = $this->sizegetOptions( $database, $languageId );

  	
 

$MenStdSizeValues = array(
	6,
	7,
	8,
	9,
...
$v++;
	...
	if ($row['default_value'] == 7)
		{
		$row['default_value'] = 7;
		}

	if ($row['option_value'] == 7)
		{
		$row['option_value'] = 7;
		}
	...
	if (is_numeric($row['option_value']))
		{
		if (!$checkOptionValueExists)
			{
			$checkOptionValueExists = array(
				$row['option_value']
			);
			}
		  else
			{
			array_push($checkOptionValueExists, $row['option_value']);
			}
		}



if ( trim($row['option_value'])=="XS" || trim($row['option_value'])=="S" || 
trim($row['option_value'])=="M" || trim($row['option_value'])=="L" || trim($row['option_value'])=="XL" || trim($row['option_value'])=="XXL"){
...
$MenStdSizeValues = array('XS','S','M','L','M','XL','XXL');

$WomenStdSizeValues = array('XS','S','M','L','M','XL','XXL');


$kidsStdSizeValues = array('XS','S','M','L','M','XL','XXL');
	...
	if (trim($row['option_name'] == "Brand"))
		...
		$BrandArray['option_name'] = $row['option_name'];
		$BrandArray['type'] = $row['type'];
		$BrandArray['option_value'] = $row['default_value'] ? $row['default_value'] : $row['option_value'];
	...
	if (trim($row['option_name'] == "Gender") && trim($row['option_value'] != "Unisex"))
		...
		if ($row['option_value'] == "Women")
			{

			$PendingStdSizeValues = array_diff($WomenStdSizeValues, $checkOptionValueExists);



			}

		if ($row['option_value'] == "Kids")
			{

			

			$PendingStdSizeValues = array_diff($kidsStdSizeValues, $checkOptionValueExists);
		...
		if (count($PendingStdSizeValues) != 0)
			{
	
			foreach($PendingStdSizeValues as $value)
				{
		
	
				$query = "SELECT `option_value_id` FROM `" . DB_PREFIX . "option_value_description` WHERE `name` =$value";
