        ...
        <div id="tab-payment" class="vtabs-content">
          <table class="form">
            ...
            <tr>
              ...
              <td><input type="text" name="payment_address_3" value="<?php echo $payment_address_2; ?>" disabled /></td>
            </tr> 
            <tr>
          ...
          <table class="list">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="left"><input type="hidden" id="tempvar"><select name="order_status_id" id="order_status_id" onchange="gettrackorderdetails();">
               ...
               <tr id="trackorderrow">
                <td class="left"><?php echo $entry_track_order; ?></td>
                <td class="left"><select name="order_track" id="order_track">
                    <!--<option value="bluedart">Bluedart</option> -->
                   <option value="dtdc">DTDC</option>
                    <option value="fedex">Fedex</option>
                    <option value="trackcourier">Trackcourier</option>
                    <option value="aramex">Aramex</option> 
                    </select></td>
              </tr>
              <tr id="waybillrow">
                <td class="left"><span class="required">*</span><?php echo $entry_way_billno; ?></td>
                <td class="left"><input type="text" id="waybillno" name="waybillno" value="" /><span id="waybillno_err"></span></td>
              </tr> 
  <tr>
                <td class="left">Would you like to send sms?</td>
                <td class="left"><input type="radio" name="sendsms" value="yes" onchange="enablebtn();"/>&nbsp;Yes&nbsp;&nbsp;<input type="radio" name="sendsms" value="no" checked="checked" onchange="enablebtn();"/>&nbsp;No &nbsp;&nbsp;&nbsp;
                <a class="button" id="button-sendsms" onclick="javascript:;sendsmsinfo();"  >Send</a>   
                  </td>
              </tr>
             ...
             <?php if($smsresults){ ?>
               <tr>
                <td class="left">Previous SMS </td> 
                <td class="left"><p><?php echo $smsresults[0]['sms']; ?></p></td>
              </tr>
              <?php } ?>
...
$( document ).ready(function() {
    ...
    $("#button-sendsms").prop('disabled', true);  
});

function sendsmsinfo()//send sms for the user
{ 
  var sendsms=$("input[name=sendsms]:checked").val();   
  var orderstatusid=$("#order_status_id").val();  
  var orderid='<?php echo $orderid; ?>'; 
  var order_track='';
   var waybillno='';
  var payment_address_2=$("input[name=payment_address_2]").val();   
  var tempvar=$("#tempvar").val();
  var flag=1; 
    ...
    if(tempvar==1)
  {
    if($("#waybillno").val()=='')
    {
      $("#waybillno_err").html('This field is required');
      $("#waybillno_err").css('color','#F00'); 
      flag=0;
    }
    else
    {
...
if(flag==1)
{
  ...
  $.ajax({ 
        ...
        url: 'index.php?route=sale/order/sendordersms&token=<?php echo $token; ?>',     
        data: {  
        ...
        success: function(resp){
         ...
         if(resp==1)
         {
          ...
          alert('Sorry, SMS can not be provided for this order status'); 
         }
...
function gettrackorderdetails()//get order track details
{
  ...
  $.ajax({ 
        ...
        url: 'index.php?route=sale/order/getorderstatusdetails&token=<?php echo $token; ?>',    
        data: {  
            orderstatusid:orderstatusid,
         },
        
        success: function(resp){  
       var respdata=resp.trim();
...
else{$("#button-sendsms").prop('disabled', true); }
}
