		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html'){
					...
					<a  id="knew1" class="dropdown-toggle" data-toggle="dropdown" >';
					
					if( $menu['image']){ $output .= '<span onclick="showlink(\''.$this->getLink( $menu ).'\')" class="menu-icon" style="background:url(\''.$this->shopUrl."image/".$menu['image'].'\') no-repeat;">';	}
					if($menu['show_title']) {
					...
					$output .= "<b class=\"caret\" href=\"void(0)\"></b>";
...
if($menu['menu_class'] == 'firstlink' && isset($_REQUEST['b']) =='1'){
					$output .= '<li class="'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>
					
<a id="knew_'.$menu['menu_class'].'" href="'.$this->getLink( $menu ).'">';

}
else if($menu['menu_class'] == 'hotdeal' && isset($_REQUEST['link']) =='d' )
{
$output .= '<li class="'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>
...
else if($menu['menu_class'] == 'contact' && isset($_REQUEST['links']) =='c' )
{
$output .= '<li class="'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>
...
if($menu['menu_class'] == 'secondlink' &&  isset($_REQUEST['c']) =='1'){
		$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					<a id="knew1_'.$menu['menu_class'].'" href="'.$this->getLink( $menu ).'" class="dropdown-toggle" data-toggle="dropdown">';
}
 
else if($menu['menu_class'] == 'thirdlink' && isset($_REQUEST['d']) =='1')
{
$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					<a id="knew1_'.$menu['menu_class'].'" href="'.$this->getLink( $menu ).'" class="dropdown-toggle" data-toggle="dropdown">';
...
else if($menu['menu_class'] == 'four' && isset($_REQUEST['e']) =='1')
{
$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					<a id="knew1_'.$menu['menu_class'].'" href="'.$this->getLink( $menu ).'" class="dropdown-toggle" data-toggle="dropdown">';
...
else if($menu['menu_class'] == 'full-width' && isset($_REQUEST['f']) =='1')
{
$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					<a id="knew1_'.$menu['menu_class'].'" href="'.$this->getLink( $menu ).'" class="dropdown-toggle" data-toggle="dropdown">';
...
$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					...
					$output .= '<span onclick="showlink(\''.$this->getLink( $menu ).'\')" class="menu-title">'.$menu['title']."</span>";
					if( $menu['description'] ){
						$output .= '<span class="menu-desc">' . $menu['description'] . "</span>";
					}
					if( $menu['image']){ $output .= '</span>';	}
					$output .= "<b class=\"caret\" href=\"void(0)\"></b></a>";
	...
	public function renderMenuContent( $menu , $level ){
		...
		if( $this->hasChild($menu['megamenu_id']) ){
			$s_value="'".$this->getLink( $menu )."'";
			$output .= '<li class="parent dropdown-submenu'.$menu['menu_class'].'" '.$this->renderAttrs($menu). '>';
			if( $menu['show_title'] ){
				$output .= '<a id="knew5" class="dropdown-toggle" data-toggle="dropdown" >';
				$t = '%s';
				if( $menu['image']){ $output .= '<span onclick="showlink(\''.$this->getLink( $menu ).'\')" class="menu-icon" style="background:url(\''.$this->shopUrl."image/".$menu['image'].'\') no-repeat;">';	}
				$output .= '<span class="menu-title">'.$menu['title']."</span>";
				if( $menu['description'] ){
			...
			$output .= '<li class="'.$menu['menu_class'].'" '.$this->renderAttrs($menu).'>';
			if( $menu['show_title'] ){ 
				$output .= '<a id="knew6" id="knew" href="'.$this->getLink( $menu ).'">';
	...
	public function getproductcategoryinfos($tproductid)//get product category infos
{
	$categories=array();
	$query1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = $tproductid order by category_id asc");  
	...
	foreach($query1->rows as $result)
	{
		...
		$query2 = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_description WHERE category_id = $catid"); 
	...
	public function getcodstatus($zipcode)
	{
		
		$query = $this->db->query("SELECT pincode FROM " . DB_PREFIX . "zipcode WHERE pincode = ".$zipcode." and (fedex_cod='yes' or aramex_cod='yes' or delhivery_cod='yes')");   
		...
		if ($query->num_rows > 0 ) {
			...
			return $query->num_rows;
		 	...
		 	$query1 = $this->db->query("SELECT pincode FROM " . DB_PREFIX . "zipcode WHERE pincode = ".$zipcode."");
		 		if ($query1->num_rows > 0 ) { return 2;}
		  else return 0;} 
	}
	public function getstatecityinfos($zipcode)
{

		$query = $this->db->query("SELECT * FROM pincode WHERE pincode = '".$zipcode."' limit 1");   
...
public function getstatecodeinfos($statename)
{
	$query1 = $this->db->query("SELECT country_id FROM " . DB_PREFIX . "country WHERE name = 'India'"); 
	

	$query = $this->db->query("SELECT zone_id FROM " . DB_PREFIX . "zone WHERE name = '".$statename."' and country_id=".$query1->row['country_id']."");   
